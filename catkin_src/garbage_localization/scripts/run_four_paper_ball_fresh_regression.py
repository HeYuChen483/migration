#!/usr/bin/env python3
"""Fresh Gazebo/RViz regression for the four_paper_ball RoboCup task."""
...
EXPECTED_ROOMS = set(['living_room', 'kitchen', 'bedroom', 'dining_room'])
EXPECTED_PICKUP_CLASSES = set(['bottle', 'paper_ball', 'box'])
DEFAULT_LOG_DIR_PREFIX = '/tmp/robocup_four_paper_ball_fresh_'
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(
        description='Run one fresh Gazebo/RViz four_paper_ball full-flow regression.')
    ...
    parser.add_argument('--visit-count', type=int, default=EXPECTED_TARGET_COUNT)
...
def matching_fresh_regression_pids():
    patterns = (
...
def cleanup_fresh_regression_processes(grace_seconds=12.0):
    pids = matching_fresh_regression_pids()
    if not pids:
        return
    process_groups = set()
    for pid in pids:
    ...
    while time.monotonic() < deadline:
        if not matching_fresh_regression_pids():
            break
        time.sleep(0.2)
    for pid in matching_fresh_regression_pids():
        try:
...
def assert_four_paper_ball_passed(args, output):
    ...
    require(summary.get('visited_targets') == args.visit_count,
            'Expected visited_targets={}, got {}'.format(
                args.visit_count, summary.get('visited_targets')))
    completed_rooms = set(summary.get('completed_rooms', []))
    require(EXPECTED_ROOMS.issubset(completed_rooms),
            'Expected completed rooms {}, got {}'.format(
                sorted(EXPECTED_ROOMS), sorted(completed_rooms)))
    require(final_exit.get('status') == 'EXIT_REACHED',
            'Expected final_exit.status EXIT_REACHED, got {}'.format(
                final_exit.get('status')))

    targets = summary.get('targets', [])
    require(len(targets) == args.visit_count,
            'Expected {} target summaries, got {}'.format(args.visit_count, len(targets)))
    rooms = set()
    classes = set()
    dropoff_failures = []
    preconfirm_failures = []
    for index, target in enumerate(targets, 1):
        ...
        if status != 'DROPOFF_REACHED':
            dropoff_failures.append((index, room, class_name, status))
        pre_status, pre_source = pre_pickup_dropoff_status(target)
        if pre_status != 'DROPOFF_CONFIRMED' or pre_source != 'current_perception':
            preconfirm_failures.append((index, room, class_name, pre_status, pre_source))
    require(EXPECTED_ROOMS.issubset(rooms),
            'Expected target rooms {}, got {}'.format(sorted(EXPECTED_ROOMS), sorted(rooms)))
    require(classes.issubset(EXPECTED_PICKUP_CLASSES),
            'Unexpected pickup target classes: {}'.format(sorted(classes)))
    ...
    kitchen_targets = [target for target in targets if target.get('room') == 'kitchen']
    require(kitchen_targets, 'No kitchen target summary found')
    kitchen_class = target_class_name(kitchen_targets[0])
    require(kitchen_class == 'bottle',
            'Expected kitchen target class bottle, got {}'.format(kitchen_class))

    return {
        'status': summary.get('status'),
        'visited_targets': summary.get('visited_targets'),
        'completed_rooms': sorted(completed_rooms),
...
def run_task_command(args, log_dir):
    ...
    result = assert_four_paper_ball_passed(args, output)
...
def run_regression(args):
    ...
    success = False
    try:
        cleanup_fresh_regression_processes()
...
def main(argv=None):
    ...
    print(json.dumps({
        'status': 'FOUR_PAPER_BALL_FRESH_REGRESSION_PASSED',
        'scenario': SCENARIO,
        'result': result,
